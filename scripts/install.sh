...
while true; do
    ...
    if nixos-enter --root /mnt -c 'passwd root'; then
        break
    else
        echo -e "${RED}Failed to set root password. Please try again.${NC}"
    fi
done

# 为普通用户设置密码
NORMAL_USERS=$(nixos-enter --root /mnt -c 'getent passwd' | awk -F: '$3 >= 1000 && $3 < 65534 {print $1}')

for user in $NORMAL_USERS; do
    while true; do
        echo -e "\nPlease set password for user ${GREEN}$user${NC}:"
        if nixos-enter --root /mnt -c "passwd $user"; then
            break
        else
            echo -e "${RED}Failed to set password for $user. Please try again.${NC}"
        fi
    done
done
